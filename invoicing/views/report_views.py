...
class CustomerStatement(ConfigMixin,
                        ...
                        TemplateView):
    template_name = os.path.join(
        'invoicing', 'reports', 'customer_statement.html')
    page_length = 20
    ...
    def get_context_data(self, *args, **kwargs):
        context = super(CustomerStatement, self).get_context_data(
            *args, **kwargs)
...
class InvoiceAgingReport(ConfigMixin, MultiPageDocument, TemplateView):
    ...
    def get_context_data(self, *args, **kwargs):
        context = super(InvoiceAgingReport, self).get_context_data(
            *args, **kwargs)
        context['pdf_link'] = True
...
class SalesReportView(ContextMixin,
    ...
    def common_context(context, start, end):
        ...
        context["pie_chart_2"] = plot_sales_by_products_and_services(
            start, end)

        customer_list = set([i.customer for i in Invoice.objects.filter(
            date__gte=start, date__lte=end, draft=False)])
