...
class Dashboard(ContextMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if config is None:
            ...
            config = models.EmployeesSettings.objects.create(
                is_configured=True)

        if config.is_configured:
            ungraded = models.Employee.objects.filter(
                pay_grade__isnull=True).count()
            if ungraded > 0:
                messages.info(
                    request, f'{ungraded} employees have no paygrades, please review the employees and assign paygrades to them for payroll to work')
