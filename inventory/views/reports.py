...
class InventoryReport(ConfigMixin, MultiPageDocument, TemplateView):
    ...
    def get_context_data(self, *args, **kwargs):
        context = super(InventoryReport, self).get_context_data(
            *args, **kwargs)
        context['items'] = models.WareHouseItem.objects.filter(
            item__type=0, item__active=True)
...
class OutstandingOrderReport(ConfigMixin, MultiPageDocument, TemplateView):
    template_name = os.path.join(
        'inventory', 'reports', 'outstanding_orders', 'report.html')
    page_length = 20
    ...
    def get_context_data(self, *args, **kwargs):
        context = super(OutstandingOrderReport,
...
class TransactionByVendorReportView(ContextMixin, ConfigMixin, TemplateView):
    template_name = os.path.join(
        'inventory', 'reports', 'vendor_transactions', 'report.html')
