...
def profit_and_loss_csv(request, start=None, end=None):
    ...
    writer = csv.writer(response)
    string = render_to_string(
        ProfitAndLossReport.template_name, ProfitAndLossReport.common_context({}, start, end))
...
def journal_csv_report(request, start=None, end=None, journal=None):
    ...
    for row in entries:
        for debit in row.debits:
            writer.writerow(
                [row.pk, row.date, '', debit.account, debit.amount])
        for credit in row.credits:
            writer.writerow(
                [row.pk, row.date, credit.account, '', credit.amount])
